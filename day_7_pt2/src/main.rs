...
// Converting Cards to a Number Representation
fn convert_card_to_number(c: char) -> usize {
    match c {
      'A' => 14,
      'K' => 13,
      'Q' => 12,
      'J' => 0, // Jokers are now worth 0
      'T' => 10,
      '9' => 9,
      '8' => 8,
      '7' => 7,
...
fn calculate_strength(input: String) -> usize {
    ...
    // If there are any Joker's in a Hand, add their counts to the card with the highest count
    if card_counts.contains_key(&'J') {
        ...
        if joker_count < 5 {
            // Finding the card with the highest count and ensuring that it is not the Joker
            let highest_card = *card_counts.iter()
                .filter(|(&card, _)| card != 'J')   // Ensuring that the Joker is not the highest card
            ...
            *card_counts.entry(highest_card).or_insert(0) += joker_count;
            ...
            // Handle the case where there are 5 Jokers
            strength = 7; // Five of a Kind with Jokers
        }
    }
