...
fn extrapolate_numbers(numbers: Vec<i64>) -> i64{
    ...
    for mut delta in deltas {
        delta_value = delta[0] - delta_value;   // Calculating the Extrapolated Number
        delta.insert(0, delta_value);       // Inserting the Extrapolated Number to the front of the Delta Vector
        extrapolated_numbers.push(delta_value);     // Pushing the Extrapolated Number to the Extrapolated Numbers Vector
